...
__constant__ float const_mask[3136];
// __constant__ float const_mask[5000];
...
{
    // use const mask instead of shared memory
    ...
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        ...
        if (row < numCRows && col < numCColumns) {
            for (int i = 0; i < TILE_WIDTH; i++) {
                val += const_mask[(size_t) row * numAColumns + tileId * TILE_WIDTH + i] * tileB[i][tx];
            }
        }
        __syncthreads();
    }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // std::cout << "Mask size in bytes: " << mask_size << " (" << mask_size / sizeof(float) << " elements)" << std::endl;
    ...
    // const mask
    cudaMemcpyToSymbol(const_mask, host_mask, mask_size, 0, cudaMemcpyHostToDevice);
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // matrixMultiplyShared<<<matmul_kernel_grid_dim, matmul_kernel_block_dim>>>(const_mask, unrolled_matrix, matmul_output, numARows, numAColumns,
    //     numBRows, numBColumns, numCRows, numCColumns);
